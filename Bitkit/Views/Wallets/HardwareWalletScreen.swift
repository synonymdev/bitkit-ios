...
struct HardwareWalletScreen: View {
    ...
    /// The vendor's shared upright device, transformed to match the Figma "Wallet Overview" visual:
    /// cover-filled into a square, rotated -15°, and clipped to a 256pt box that bleeds off the
    /// screen's trailing edge. Reuses the generic device asset (no screen-specific crop is baked
    /// in), so the same image can be adapted elsewhere with SwiftUI.
