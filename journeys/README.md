...
| [pubky-auth](pubky-auth) | 1 | OS handoff into watch-only consent; local Pubky identity required |
...
but `onOpenURL` in `Bitkit/MainNavView.swift` only handles web URLs, Pubky auth requests and callbacks,
